...
- `pdfanno highlight INPUT NEEDLE` — literal search + quad-based highlight,
  with `--dry-run`, `--json`, `--color`, `--pages`, `--ignore-case`, `--sidecar`,
  `--in-place`.
...
- `pdfanno extract --format json|markdown` — export annotations.
...
- Case-sensitive literal matching degrades to PyMuPDF's default (ASCII-
  insensitive) behavior. Strict case-sensitive literal lands in v1.5.
...
- `LICENSE` is a placeholder declaration; **before the PyPI release the full
  verbatim AGPL-3.0 text must be pasted in**.
